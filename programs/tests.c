...
// #define DMA_BASE (0x80000000 + 0x20000000) dram address
...
int main(int argc, char const *argv[]) {
    ...
    *dma_conf = 42;
    ...
    // assert(*dma_src == 42);
    // assert(*dma_dst == 42);
    // assert(*dma_num_bytes == 42);
    // assert(*dma_conf == 42);
    
    // /*
    //  * Test DMA transfer
    //  */
    // printf("Initiate dma request\n");

    // // allocate DMA_TRANSFER_SIZE bytes
    // uint64_t *src = (uint64_t*)malloc(DMA_TRANSFER_SIZE);
    // uint64_t *dst = (uint64_t*)calloc(DMA_TRANSFER_SIZE / sizeof(uint64_t), sizeof(uint64_t));
